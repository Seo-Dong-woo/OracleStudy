 ...
 *          전송 ======결과값 받기
 *     | 결과값을 받아서 화면 출력(HTML)
 *   *** 오라클 SQL문장과 자바에서 전송하는 SQL문장이 다른 것이 있다
 *                    95%는 동일 => 5%가 다름
 *                    => LIKE
 *                    => 오라클/MySQL
 *   1) 연결(송수신) => 드라이버 설정
...
public class EmpDAO {
	...
	// 기능 수행 => 메소드 => 테이블 1개다 => VO, DAO
	// SQL문장 전송
	// emp에서 데이터 출력 => 사번, 이름, 입사일, 직위, 급여
	public void empListDate()
	{
		try
		{
			// 1. 오라클 연결
	...
	// 사원의 이름, 직위, 급여, 입사일, 성과급(성과급이 있는 사원 0은 제외)
	...
	public void subqueryNotData()
	{
		try
		{
			...
			String sql="SELECT ROUND(AVG(sal)) "
					 + "FROM emp";
			ps=conn.prepareStatement(sql); // sql문장은 한 개만 전송 가능
			ResultSet rs=ps.executeQuery();
			rs.next(); // 커서의 위치 변경
			int avg=rs.getInt(1);
			rs.close();
