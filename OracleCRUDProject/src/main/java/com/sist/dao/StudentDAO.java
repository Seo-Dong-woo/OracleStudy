	...
	public List<StudentVO> stdListData()
	{
		...
		try
		{
			...
			String sql="SELECT hakbun, name, kor, eng, math,"
					 ...
					 + "FROM student";
			// RANK()는 자동 정렬
			// => 웹은 모든 데이터가 공유 => 데이터는 반드시 오라클에 저장된
			// => VueJS(Vuex)/ReactJS(Redux)
			//     JSP         JSP    ----- Spring
			// =====> 데이터가 없는 상태(데이터 관리 프로그램)
			// Back-End(SQL), Front-End(JSON/SJONP)
			// VO => {}, List => [] => [{}, {}, {}...]
			
			// XML => JSON ==> Ajax
			
			// 3. 오라클 SQL전송
			ps=conn.prepareStatement(sql); // 실행은 안 된 상태
			// 4. SQL문장 실행
			ResultSet rs=ps.executeQuery(); // 실행 완료 실행된 결과가 rs에 있음
			while(rs.next())
			{
